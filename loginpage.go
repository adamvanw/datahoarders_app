...
func InitializeLoginPage(windowSize rl.Vector2) ([]TextBox, []Button) {
	...
	var address TextBox = NewTextBox(layout[0], "IP Address (defaults to localhost)")
	address.text = "localhost"
	var port TextBox = NewTextBox(layout[1], "Port (defaults to 3306)")
	port.text = "3306"
	var user TextBox = NewTextBox(layout[2], "User (defaults to root)")
	user.text = "root"
	var password TextBox = NewTextBox(layout[3], "Password (defaults to football11)")
